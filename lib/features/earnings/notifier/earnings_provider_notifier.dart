...
class EarningsProviderNotifier
    extends AutoDisposeNotifier<EarningsProviderState> {
  late final ApiClient _apiClient;

  @override
  EarningsProviderState build() {
    _apiClient = ref.read(apiClientProvider);
    Future<dynamic>.microtask(() {
      _fetchSummary();
      _fetchEarnings();
    });
    return const EarningsProviderState(isLoading: true);
  }
  ...
  Future<void> _fetchSummary() async {
    ...
    try {
      final EarningsProviderDashboardResponse response = await _apiClient
          .handleRequest<EarningsProviderDashboardResponse>(
            httpMethod: HttpMethod.get,
            endpoint: ApiEndpoints.providerEarnings,
            fromJson: EarningsProviderDashboardResponse.fromJson,
          );
      state = state.copyWith(summary: response.data);
      if (response.data.onboardingPolicy != null) {
        ref
            .read(stripeConnectProvider.notifier)
    ...
    try {
      final Map<String, dynamic> response = await _apiClient
          .handleRequest<Map<String, dynamic>>(
            httpMethod: HttpMethod.get,
            endpoint: ApiEndpoints.providerEarningsTransaction,
  ...
  Future<void> withdrawRequest(num amount) async {
    try {
      final Map<String, dynamic> response = await _apiClient.handleRequest(
        ...
        endpoint: ApiEndpoints.providerWithdraw,
