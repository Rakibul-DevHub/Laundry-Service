...
class JobQrScannerScreen extends ConsumerStatefulWidget {
  final String jobId;

  const JobQrScannerScreen({
    super.key,
    required this.jobId,
  });

  @override
  ConsumerState<JobQrScannerScreen> createState() => _JobQrScannerScreenState();
}

class _JobQrScannerScreenState extends ConsumerState<JobQrScannerScreen> {
  ...
  Future<void> _verifyScannedCode() async {
    ...
    try {
      final JobsDetailsNotifier notifier = ref.read(
        jobsDetailsProvider(widget.jobId).notifier,
      );
      ...
      if (mounted) {
        ...
        // Keep the scanned code so user can retry verify or re scan
      }
    } finally {
      if (mounted) {
